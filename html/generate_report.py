...
PROFILE_CSV_PATTERN = re.compile(r"profile_.*\.csv$")
...
def read_profile_csvs():
    """Read all profile benchmark CSV files from results/profiles/."""
    records: list[ProfileRecord] = []
    if not PROFILE_RESULTS.exists():
        return records

    print(f"[ Info ] Scanning {PROFILE_RESULTS.name}/ for profile results...")

    for res_file in sorted(PROFILE_RESULTS.iterdir()):
        if not res_file.is_file() or not PROFILE_CSV_PATTERN.search(res_file.name):
            continue
            ...
            with res_file.open("r", newline="") as fh:
                ...
                for row in reader:
                    rec = ProfileRecord(
                        ...
                        latency_median_ms=parse_float(row.get("Latency_Median_ms")),
                        latency_avg_ms=parse_float(row.get("Latency_Avg_ms")),
                        throughput_fps=parse_float(row.get("Throughput_FPS")),
                        target_latency_ms=parse_float(row.get("Target_Latency_ms")),
                        target_fps=parse_float(row.get("Target_FPS")),
                        latency_pass=row.get("Latency_Pass", ""),
                        throughput_pass=row.get("Throughput_Pass", ""),
                        rationale=row.get("Rationale", ""),
                    )
                    records.append(rec)
        except Exception as e:
            print(f"[ Warning ] Failed to parse profile CSV {res_file.name}: {e}")
            continue

    return records


def aggregate_profiles(records: list[ProfileRecord]):
    """Group profile records by profile name and produce a summary per profile."""
    profiles: dict[str, list[dict]] = defaultdict(list)
    for r in records:
        profiles[r.profile].append(asdict(r))

    summary = []
    for profile_name, benchmarks in profiles.items():
        total = len(benchmarks)
        passed = sum(
            1 for b in benchmarks
            if (b["latency_pass"] == "PASS" or not b["target_latency_ms"])
            and (b["throughput_pass"] == "PASS" or not b["target_fps"])
            and (b["latency_pass"] or b["throughput_pass"])
        )
        failed = sum(
            1 for b in benchmarks
            if b["latency_pass"] == "FAIL" or b["throughput_pass"] == "FAIL"
        )
        devices = sorted(set(b["device"] for b in benchmarks if b["device"]))
        summary.append({
...
def write_data_json(summary, raw_records, profile_summary=None):
...
def main():
    ...
    records = read_csvs()
    profile_records = read_profile_csvs()

    if not records and not profile_records:
        print("[ Error ] No benchmark CSV files found in results/. Run benchmarks first.")
        return 1

    summary = aggregate(records) if records else []
    profile_summary = aggregate_profiles(profile_records) if profile_records else []
    write_data_json(summary, records, profile_summary)
    
    print(f"[ Info ] Generated data file: {DATA_JSON}")
    print(f"[ Info ] Dashboard ready at: {HTML_DIR / 'index.html'}")
    print(f"[ Info ] Processed {len(records)} pipeline records into {len(summary)} summary entries")
    if profile_records:
        print(f"[ Info ] Processed {len(profile_records)} profile benchmark records across {len(profile_summary)} profile(s)")
